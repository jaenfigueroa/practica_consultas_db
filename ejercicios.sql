...
-- 2. Seleccionar los campos id_cliente, nombres, apellidos, dirección de la tabla cliente.
SELECT ID_CLIENTE, NOMBRES, APELLIDOS, DIRECCION FROM CLIENTE


-- 3. Seleccionar los campos id_producto, descripción y precio_venta de la tabla producto utilizando Alias P.
SELECT P.ID_PRODUCTO, P.DESCRIPCION, P.PRECIO_VENTA FROM PRODUCTO P


-- 4. Seleccionar los campos id_cliente, nombres y apellidos concatenados, fono, email de la tabla cliente.
-- Utilizar Alias y cambiar el nombre de las columnas de respuesta por Código, Cliente, Teléfono, Correo.
SELECT ID_CLIENTE AS CODIGO, NOMBRES + ' ' + APELLIDOS AS CLIENTE, FONO AS TELEFONO, EMAIL AS CORREO FROM CLIENTE


-- 5. Seleccione todos distintos tipos de distrito (id_distrito) que aparecen en la tabla cliente. Utilizar
-- distinct.
...
-- alias y sentencia where.
SELECT B.* FROM DETALLEBOLETA as B WHERE CANTIDAD>5


-- 8. Mostrar todos los campos de la tabla producto, pero de aquellos cuyo código del producto sea
-- PRO002 y PRO004. Utilizar Alias, sentencia Where y operadores de comparación y lógicos.
SELECT * FROM PRODUCTO WHERE ID_PRODUCTO IN ('PRO002', 'PRO004')



-- 9. Seleccionar todos los campos de la tabla cliente, pero de aquellos cuyo código de distrito sea
-- D006. Utilizar Alias y operador de comparación.
SELECT C.* FROM CLIENTE AS C WHERE C.ID_DISTRITO='D006'


-- 10. Seleccionar id_producto, descripción y precio_venta de la tabla producto, pero de aquellos cuyo
-- precio_venta esté entre 30 y 100. Utilizar alias, operadores de comparación y lógicos.
SELECT P.ID_PRODUCTO, P.DESCRIPCION, P.PRECIO_VENTA FROM PRODUCTO AS P WHERE P.PRECIO_VENTA BETWEEN 30 AND 100



-- 11. Seleccionar todos los campos de la tabla cliente a excepción de los clientes de distrito D003 y
-- D010. Utilizar alias, operadores de comparación y operador lógico NOT
SELECT C.* FROM CLIENTE AS C WHERE C.ID_DISTRITO NOT IN ('D003', 'D010')


-- 12. Seleccionar todos los campos de la tabla Empleado cuyo nombre comienza con R y apellidos con
-- T. Utilizar alias, operadores lógicos y sentencia LIKE
SELECT * FROM EMPLEADO WHERE NOMBRES LIKE 'R%' AND APELLIDOS LIKE 'T%'



-- 13. Seleccionar todos los campos de la tabla Empleado cuya segunda letra de apellidos es A y estado
-- civil soltero (S). Utilizar alias, operadores lógicos y sentencia LIKE
SELECT * FROM EMPLEADO WHERE APELLIDOS LIKE '_A%' AND ESTADO_CIVIL='S'


-- 14. Mostrar todos los campos de la tabla producto, pero de aquellos que su stock actual esté entre
-- 60 y 120. Utilizar alias y sentencia BETWEEN

SELECT P.* FROM PRODUCTO AS P WHERE P.STOCK_ACTUAL BETWEEN 60 AND 120


-- 15. Mostrar todos los campos de la tabla Empleado, pero de aquellos que sean de los distritos D005,
-- D007 y D009. Utilizar Alias y sentencia IN
-- Base de datos
SELECT E.* FROM EMPLEADO AS E WHERE E.ID_DISTRITO IN ('D005', 'D007', 'D009')
...
-- stock_actual mayor o igual a 30.
SELECT ID_PRODUCTO, DESCRIPCION, PRECIO_VENTA, STOCK_ACTUAL
FROM PRODUCTO 
WHERE (DESCRIPCION LIKE 'A%' OR DESCRIPCION LIKE 'M%') AND (PRECIO_VENTA>=15) AND (STOCK_ACTUAL>=30)


-- 17. Mostrar todas las ventas emitidas desde 01/01/2016 hasta 31/12/2016.
SELECT * FROM BOLETA WHERE FECHA_EMI BETWEEN '01-01-2016' AND '31-12-2016'


-- 18. Mostrar todos los campos de la tabla producto, pero de aquellos que su categoría sea C10 o C05.
SELECT * FROM PRODUCTO WHERE COD_CATE IN ('C10', 'C05')
...
-- sueldo_básico mayor a 1000 y cuyo email termina en GMAIL.COM
SELECT * FROM EMPLEADO
...
WHERE
	ESTADO_CIVIL='C' AND
	NIVEL_EDUCA IN ('SUPERIOR', 'SECUNDARIA') AND
	SUELDO_BASICO>1000 AND
...
-- la letra E en la penúltima posición.
SELECT COD_EMPLE, NOMBRES, APELLIDOS, ESTADO_CIVIL, NIVEL_EDUCA, EMAIL, SUELDO_BASICO
FROM EMPLEADO
WHERE
	ESTADO_CIVIL='S' AND
	SUELDO_BASICO>980 AND
	APELLIDOS LIKE '%E_'


-- 21. Mostrar todas las boletas que pertenece a la cliente ANETH LUANA (Utilice su código)
SELECT * FROM BOLETA
WHERE ID_CLIENTE = (SELECT ID_CLIENTE FROM CLIENTE WHERE NOMBRES = 'ANETH LUANA');
